export const metadata = {
  title: "PPF Calculator India — Public Provident Fund Calculator & Returns",
  description: "Estimate your PPF maturity amount, tax-free returns, and long-term savings with our free India PPF calculator.",
  openGraph: {
    title: "Free PPF Calculator — Calculate PPF Returns & Maturity Amount",
    description: "Plan your PPF investment and calculate maturity amount instantly.",
    url: "https://www.moneytool.in/ppf-calculator",
    images: [
      {
        url: "https://www.moneytool.in/og-images/ppf-calculator.png",
        width: 1200,
        height: 630,
        alt: "PPF Calculator for India",
      },
    ],
  },
  twitter: {
    card: "summary_large_image",
    title: "Free PPF Calculator — Calculate PPF Returns",
    description: "Plan your PPF investment and calculate maturity amount instantly.",
    image: "https://www.moneytool.in/og-images/ppf-calculator.png",
  },
  alternates: {
    canonical: "https://www.moneytool.in/ppf-calculator",
  },
};
